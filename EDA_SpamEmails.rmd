...
This is an Exploratory Data Analysis of the `email` dataset from the `openintro` package. T
...
library(openintro); library(tidyverse); library(magrittr); library(corrplot); library(lubridate)
```
...
email %>% 
  mutate(hour = update(time, yday = 1)) %>% 
  ggplot(aes(hour)) +
    geom_freqpoly(binwidth = 300) + theme(axis.text.x = element_text(angle = 90)) + facet_grid(~ spam)
```

**So there doesn't seem to be a particular time of the day when there is a spike in spam emails** <br />
